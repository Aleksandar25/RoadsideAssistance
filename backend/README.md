...
24 теста (Jest + Supertest), покриващи регистрация/вход/редакция на профил, създаване/приемане/отказ на заявки, ролеви ограничения, прикачване на автомобил към заявка и оценяване на доставчик след приключена заявка. Изпълняват се срещу in-memory MongoDB — не докосват реалната dev база.
...
| PATCH | `/me` | Private | `{name, phone}` | Редакция на име/телефон (не и имейл/парола/роля) |
...
| PATCH | `/:id/rating` | CLIENT | `{rating (1-5), comment?}` | Еднократна оценка на доставчика, само след `COMPLETED` |
